...
export default function CaseStudy() {
    ...
    return (
      <main className="pt-32 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/portfolio" className="inline-flex items-center gap-2 hover:text-white/80 transition-colors" style={{ color: '#c5f74f' }}>
  ...
  return (
    <main className="pt-20">
      ...
      <section className="relative h-[65vh] flex items-end overflow-hidden">
        ...
        <div className="container-arch relative z-10 pb-14">
          ...
          <div className="flex flex-wrap gap-3 mb-3">
            <span className="px-3 py-1 rounded-full text-sm font-medium" style={{ background: 'rgba(197,247,79,0.12)', color: '#c5f74f' }}>{project.category}</span>
      ...
      <section className="py-12 border-b border-white/5">
        <div className="container-arch grid grid-cols-2 md:grid-cols-4 gap-6">
          {[{ l: 'Client', v: project.client }, { l: 'Category', v: project.category }, { l: 'Year', v: project.year }, { l: 'Service', v: project.filter }].map((x, i) => (
            <div key={i}>
              <p className="text-xs uppercase tracking-widest mb-1" style={{ color: '#c5f74f' }}>{x.l}</p>
      ...
      {/* Creative Direction */}
      <section className="py-16" style={{ background: 'rgba(167,139,250,0.04)' }}>
        <div className="container-arch max-w-3xl">
      ...
      <section className="py-20">
        <div className="container-arch">
          <div className="text-center mb-12">
            ...
            <h2 className="heading-serif text-3xl md:text-5xl font-bold">From Brief to <span className="text-accent-grad">Final Result</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {project.process.map((step, i) => (
              <motion.div key={i} {...fadeUp} transition={{ delay: i * 0.12 }}>
                <div className="heading-serif text-6xl font-bold opacity-10 mb-2" style={{ color: '#c5f74f' }}>0{i + 1}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {project.gallery.map((img, i) => (
              <motion.div key={i} {...fadeUp} transition={{ delay: i * 0.1 }}
                className="rounded-2xl overflow-hidden border border-white/5 hover:border-lime/20 transition-all duration-500">
      ...
      <section className="py-20">
        <div className="container-arch">
          <div className="text-center mb-12">
            ...
            <h2 className="heading-serif text-3xl md:text-5xl font-bold">Results That <span className="text-accent-grad">Speak</span></h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
            {project.results.map((r, i) => (
              <motion.div key={i} {...fadeUp} transition={{ delay: i * 0.1 }}
                className="card-arch p-8 text-center gradient-border">
                <div className="heading-serif text-4xl md:text-5xl font-bold text-accent-grad mb-2">{r.value}</div>
